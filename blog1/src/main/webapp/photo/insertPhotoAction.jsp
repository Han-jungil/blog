...
<%
	...
	*/
	...
	String path = application.getRealPath("upload"); // applicaption변수는 톰켓을 가르키는 변수!
	...
	String photoCategoryName = multiReq.getParameter("photoCategoryName");
	...
	System.out.println(photoCategoryName + " <-- photoCategoryName");
	System.out.println(photoName + " <-- photoName");
	System.out.println(photoType + " <-- photoType");
	
	//파일 업로드의 경우 100mbyte 이하의 image/gif, image/png, image/jpeg 3가지 이미지만 허용하겠다.
	if(photoType.equals("image/gif") ||photoType.equals("image/png") || photoType.equals("image/jpeg")) { //db에 저장...
		System.out.println("db에 저장");
		PhotoDao photoDao = new PhotoDao();
		Photo photo = new Photo();
		photo.setPhotoName(photoName);
		photo.setPhotoOriginalName(photoOriginalName);
		photo.setPhotoCategoryName(photoCategoryName);
		...
		File file = new File(path + "\\" + photoName); // java.io.File으로 잘못된 파일을 불러온다.
